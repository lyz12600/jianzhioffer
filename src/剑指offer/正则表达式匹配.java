package 剑指offer;

import 剑指offer.bean.ScannerHelper;

import java.util.Scanner;

/**
 * 面试题：53
 * 正则表达式匹配
 * 'a', 'a', 'a'
 * 'a', 'b', '*', 'a', 'c', '*', 'a'
 * <p>
 * Created by liyanzhen on 17/6/27.
 */
public class 正则表达式匹配 {
    public static void main(String[] args) {
        Scanner in = new ScannerHelper().getScanner("src/剑指offer/正则表达式匹配.txt");
        while (in.hasNextLine()) {
            String strings = in.nextLine();
            char[] str;
            char[] pattern;
            if (strings.equals(",")) {
                str = null;
                pattern = null;
            } else {
                str = strings.split(",")[0].toCharArray();
                pattern = strings.split(",")[1].toCharArray();
            }
            System.out.println(match(str, pattern));
        }
    }

    public static boolean match(char[] str, char[] pattern) {
        if (str == null || pattern == null) {
            return false;
        }
        int strIndex = 0;
        int patternIndex = 0;
        return matchCore(str, strIndex, pattern, patternIndex);
    }

    public static boolean matchCore(char[] str, int indexStr, char[] pattern, int indexPattern) {
        //有效性检验：str到尾，pattern到尾，匹配成功
        if (indexStr == str.length && indexPattern == pattern.length) {
            return true;
        }
        //pattern先到尾，匹配失败
        if (indexStr != str.length && indexPattern == pattern.length) {
            return false;
        }
        //模式第2个是*，且字符串第1个跟模式第1个匹配,分3种匹配模式；如不匹配，模式后移2位
        if (indexPattern + 1 < pattern.length && pattern[indexPattern + 1] == '*') {
            if ((indexStr != str.length && pattern[indexPattern] == str[indexStr])
                    || (pattern[indexPattern] == '.' && indexStr != str.length)) {//  (aVS.*)
                return matchCore(str, indexStr, pattern, indexPattern + 2)//模式后移2，视为x*匹配0个字符
                        || matchCore(str, indexStr + 1, pattern, indexPattern + 2)//视为模式匹配1个字符
                        || matchCore(str, indexStr + 1, pattern, indexPattern);//*匹配1个，再匹配str中的下一个 (aaVSa*)
            } else {
                return matchCore(str, indexStr, pattern, indexPattern + 2);
            }
        }
        //模式第2个不是*，且字符串第1个跟模式第1个匹配，则都后移1位，否则直接返回false
        if ((indexStr != str.length && pattern[indexPattern] == str[indexStr])
                || (pattern[indexPattern] == '.' && indexStr != str.length)) {
            return matchCore(str, indexStr + 1, pattern, indexPattern + 1);
        }
        return false;
    }
}
